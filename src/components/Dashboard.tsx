...
export default function Dashboard({ 
  ...
  const unreadNotifications = useMemo(() => {
    return notifications.slice(0, 5); // Active live notifications
  }, [notifications]);
  ...
  const stats = useMemo(() => {
    ...
    // 2. Money I Owe Others (Suppliers)
    let totalOwedToSuppliers = 0;
    suppliers.forEach(sup => {
      totalOwedToSuppliers += Math.max(0, sup.amount - sup.paidAmount);
    });

    // 3. Money Collected This Month (July 2026)
    const collectedThisMonth = payments
      .filter(p => p.date.startsWith('2026-07'))
      .reduce((acc, p) => acc + p.amount, 0);

    // 4. Money Paid This Month (July 2026) - Payments made to suppliers
    const paidToSuppliersThisMonth = suppliers
      .filter(s => s.createdAt.startsWith('2026-07')) // simulated
      .reduce((acc, s) => acc + s.paidAmount, 0); // simpler placeholder for suppliers
  ...
  // Chart Data Preparation: Monthly collection trend
  const collectionTrendData = useMemo(() => {
    const monthlyData: { [key: string]: { collected: number; paid: number } } = {
      ...
      '06': { collected: 60000, paid: 20000 }, // simulated historical
      '07': { collected: stats.collectedThisMonth, paid: stats.paidToSuppliersThisMonth }
    };

    // Aggregate payments by month (for 2026)
    payments.forEach(p => {
  ...
  // Outstanding vs Paid Breakdown (Pie Chart)
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800" id="dashboard-welcome-heading">Habari ya leo! 👋</h1>
          <p className="text-sm text-slate-500 mt-1">Hapa kuna muhtasari wa fedha na madeni ya biashara yako kwa leo.</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs font-mono bg-slate-100 text-slate-600 px-3 py-1.5 rounded-full flex items-center gap-1.5 border border-slate-200">
            <span className="h-2 w-2 rounded-full bg-accent animate-pulse"></span>
            Tarehe: 10 Julai, 2026 (Muda wa sasa)
          </span>
          <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition flex flex-col justify-between" id="card-customers-owe-me">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition flex flex-col justify-between" id="card-collected-this-month">
          <div className="flex justify-between items-start">
        ...
        {/* Outstanding Balance */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition flex flex-col justify-between" id="card-outstanding-balance">
          <div className="flex justify-between items-start">
        ...
        {/* Due Today */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition flex flex-col justify-between" id="card-due-today">
          <div className="flex justify-between items-start">
        ...
        {/* Overdue Debts */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition flex flex-col justify-between" id="card-overdue-debts">
          <div className="flex justify-between items-start">
        ...
        {/* Total Customers */}
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition flex flex-col justify-between" id="card-total-customers">
          <div className="flex justify-between items-start">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Jumla ya Wateja</span>
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1">Wateja kwenye mfumo wako</p>
          </div>
        </div>

        {/* Quick Action Add shortcut */}
        <div className="bg-slate-900 text-white p-6 rounded-3xl hover:bg-slate-850 transition flex flex-col justify-between cursor-pointer shadow-sm" onClick={() => setCurrentTab('customers')}>
          <div className="flex justify-between items-start">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-accent hover:underline cursor-pointer" onClick={() => setCurrentTab('calendar')}>
              Angalia Kalenda
            </span>
          </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <h3 className="text-md font-bold text-slate-800 mb-4">Kumbukumbu ya Matendo (Transaction History)</h3>
                ...
                <tr key={tx.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition">
                  <td className="py-3 px-4 font-bold">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] ${
                      ...
                      tx.actionType.includes('Deleted') ? 'bg-rose-50 text-rose-700' : 'bg-slate-100 text-slate-700'
                    }`}>
                      {tx.actionType}
                    </span>
                  </td>
