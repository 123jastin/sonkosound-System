...
import { 
  ...
  ArrowLeft, History, Printer, Building, Loader2, MapPin
} from 'lucide-react';
...
export default function SupplierManagement({
  ...
  const activeSupplierStats = useMemo(() => {
    ...
    const totalOwed = activeSupplier.amount || 0;
    const totalPaid = activeSupplier.paidAmount || 0;
  ...
  const activeSupplierProducts = useMemo(() => {
    ...
    // Fallback virtual product if there's an amount
    if ((activeSupplier.amount || 0) > 0) {
      return [{
  ...
  const activeSupplierPayments = useMemo(() => {
    ...
    if ((activeSupplier.paidAmount || 0) > 0) {
      return [{
  ...
  const handleEditSupplier = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedSupplier || !name || !phoneNumber) return;
    ...
    try {
      await api.suppliers.update(selectedSupplier.id, {
        ...
        dueDate: selectedSupplier.dueDate,
        productType,
        notes
      });
      onUpdate();
  ...
  const handleCreateSupplierProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fallback - update supplier amount directly
      const newProduct = {
        id: 'prod-' + Date.now(),
        supplierId: activeSupplier.id,
        description: newProductDesc,
      ...
      // Update supplier amount locally
      const updatedSuppliers = suppliers.map(s => {
        if (s.id === activeSupplier.id) {
          return {
            ...s,
            amount: (s.amount || 0) + Number(newProductAmount),
            products: [...(s.products || []), newProduct],
            productType: s.productType || newProductDesc
          };
        }
        return s;
      });
      ...
      setNewProductDueDate('');
      setNewProductNotes('');
      setError('Imeshindwa kuongeza mzigo kwenye seva, lakini imehifadhiwa kwa muda.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {activeSupplier && activeSupplierStats ? (
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-6 md:p-8 shadow-sm space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4">
                <button onClick={() => setSelectedSupplierId('')} className="p-2.5 hover:bg-slate-50 text-slate-500 hover:text-slate-700 rounded-2xl border border-slate-100 transition-colors mr-1">
                  <ArrowLeft size={16} />
                </button>
                <div className="h-16 w-16 rounded-2xl bg-amber-100 text-amber-800 font-extrabold text-xl flex items-center justify-center shadow-sm uppercase">
                  {getInitials(activeSupplier.name)}
                </div>
                <div>
                  ...
                  {activeSupplier.address && (
                    <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                      <MapPin size={12} /> {activeSupplier.address}
                    </p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 self-start sm:self-center">
                <button onClick={() => openEditModal(activeSupplier)} disabled={isLoading} className="py-2.5 px-4 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 transition-colors flex items-center gap-1.5 font-bold disabled:opacity-50">
                  <Edit2 size={14} /> Hariri
                </button>
                <button onClick={() => handleDeleteSupplier(activeSupplier.id, activeSupplier.name)} disabled={isLoading} className="py-2.5 px-4 rounded-xl border border-rose-200 hover:bg-rose-50 text-rose-600 transition-colors flex items-center gap-1.5 font-bold disabled:opacity-50">
                  <Trash2 size={14} /> Futa
                </button>
                <button onClick={() => setSelectedSupplierId('')} className="py-2.5 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors font-bold">
                  Orodha
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="space-y-3.5">
                ...
                {activeSupplierStats.hasDebts && (
                  <>
                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mt-3">
                      <div className="bg-emerald-600 h-full rounded-full transition-all" style={{ width: `${activeSupplierStats.percentagePaid}%` }}></div>
                    </div>
            ...
            <div className="md:col-span-2 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4 flex flex-col justify-between">
              ...
              <div className="pt-4 flex flex-wrap gap-2">
                <button onClick={() => setIsAddProductModalOpen(true)} disabled={isLoading} className="bg-accent hover:bg-accent/90 text-white font-bold py-2.5 px-4 rounded-xl flex items-center gap-1.5 shadow-sm transition disabled:opacity-50">
                  <Plus size={14} /> Deni Jipya / Ongeza Mzigo
                </button>
                {activeSupplierStats.remainingOwed > 0 && (
                  <button onClick={() => openPayModal(activeSupplier)} disabled={isLoading} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-4 rounded-xl flex items-center gap-1.5 shadow-sm transition disabled:opacity-50">
                    <CreditCard size={14} /> Lipa Deni
                  </button>
                )}
                <button onClick={() => setIsStatementOpen(true)} className="bg-slate-900 hover:bg-slate-800 text-white font-bold py-2.5 px-4 rounded-xl flex items-center gap-1.5 shadow-sm transition">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto">
                {activeSupplierProducts.length > 0 ? (
                  activeSupplierProducts.map((p, idx) => {
                    const productPayments = activeSupplierPayments.filter(pay => pay.notes && pay.notes.includes(p.description));
                          ...
                          {p.amount > 0 && (
                            <div className="mt-2 space-y-1">
                              ...
                              <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full transition-all ${remainingForProduct > 0 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{ width: `${Math.min(100, (totalPaidForProduct / p.amount) * 100)}%` }}></div>
                              </div>
                            </div>
                          )}
                        </div>
                  ...
                  <p className="text-xs text-slate-400 italic text-center py-6">Hakuna bidhaa au mizigo bado. Ongeza kwa kutumia kitufe cha "Deni Jipya".</p>
                )}
              </div>
            </div>
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-white p-5 rounded-3xl border border-slate-100 shadow-sm gap-4">
            ...
            <button onClick={() => { resetForm(); setIsAddModalOpen(true); }} disabled={isLoading} className="bg-accent hover:bg-accent/90 text-white font-semibold text-xs py-2.5 px-4 rounded-xl flex items-center gap-1.5 shadow-sm transition disabled:opacity-50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {suppliers.length > 0 ? (
              suppliers.map(sup => {
                const rem = (sup.amount || 0) - (sup.paidAmount || 0);
                const hasDebts = (sup.amount || 0) > 0;
                
                return (
                  <div key={sup.id} onClick={() => setSelectedSupplierId(sup.id)} className="bg-white rounded-3xl border border-slate-100 p-5 shadow-sm hover:shadow-md hover:border-slate-300 cursor-pointer transition flex flex-col justify-between space-y-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="h-12 w-12 rounded-xl bg-amber-100 text-amber-800 font-bold flex items-center justify-center">
                      ...
                      {hasDebts ? (
                        <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full ${rem <= 0 ? 'bg-success/10 text-success' : 'bg-rose-100 text-rose-700'}`}>
                          {rem <= 0 ? 'Safi' : 'Unpaid'}
                        </span>
                      ) : (
                    ...
                    {hasDebts ? (
                      <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 text-xs space-y-2">
                        <div className="flex justify-between font-medium">
                          ...
                          <span className="font-bold text-slate-700">TSh {(sup.amount || 0).toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between font-medium">
                          <span className="text-slate-400">Lipwa:</span>
                          <span className="font-bold text-success">TSh {(sup.paidAmount || 0).toLocaleString()}</span>
                        </div>
                      ...
                      <div className="p-3 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center">
                        <p className="text-xs text-slate-400">Hakuna madeni bado</p>
                      </div>
                    )}

                    {sup.productType && (
                      <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-accent/10 text-accent w-fit">
                        {sup.productType}
                      </span>
                    )}
                    ...
                    <div className="flex items-center justify-between border-t border-slate-50 pt-3">
                      <p className="text-[10px] text-slate-400 flex items-center gap-0.5 font-mono">
                        <Calendar size={10} /> {sup.createdAt}
                      </p>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative animate-scale-in">
            ...
            <p className="text-xs text-slate-400">Ongeza maelezo ya msambazaji. Madeni yanaweza kuongezwa baadaye.</p>
            ...
            <form onSubmit={handleAddSupplier} className="space-y-4 text-xs text-left">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Mtaa, Jiji" className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-accent focus:border-accent" />
                </div>
              </div>
              <div>
                <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Bidhaa (Product Type)</label>
                <input type="text" value={productType} onChange={(e) => setProductType(e.target.value)} placeholder="Mfano: Mizigo/Products, Huduma n.k." className="w-full p-2.5 border border-slate-200 rounded-xl bg-white focus:ring-accent focus:border-accent" />
              </div>
              <div>
                <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Maelezo ya Ziada</label>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Maelezo mengineyo..." className="w-full p-2.5 border border-slate-200 rounded-xl h-20 focus:ring-accent focus:border-accent" />
              </div>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative animate-scale-in">
            ...
            <form onSubmit={handleEditSupplier} className="space-y-4 text-xs text-left">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Anuani</label>
                  <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-accent focus:border-accent" />
                </div>
              </div>
              <div>
                <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Bidhaa (Product Type)</label>
                <input type="text" value={productType} onChange={(e) => setProductType(e.target.value)} placeholder="Mfano: Mizigo/Products" className="w-full p-2.5 border border-slate-200 rounded-xl bg-white focus:ring-accent focus:border-accent" />
              </div>
              <div>
                <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Maelezo ya Ziada</label>
      ...
      {/* MODAL: Add Supplier Product/Debt */}
      {isAddProductModalOpen && activeSupplier && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative animate-scale-in">
            ...
            <FormAIOCR label="Changanua Risiti kwa AI Camera" onSuccess={(data) => {
      ...
      {isPayModalOpen && selectedSupplier && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl relative animate-scale-in">
            <button onClick={() => { setIsPayModalOpen(false); setSelectedSupplier(null); setSelectedProductId(''); }} className="absolute top-4 right-4 p-1.5 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-50 transition">
              <X size={18} />
            </button>
            <h3 className="text-md font-bold text-slate-850">Rekodi Malipo ya Msambazaji</h3>
            <p className="text-xs text-slate-400">Lipa <strong>{selectedSupplier.name}</strong>.</p>
            <form onSubmit={handlePaySupplier} className="space-y-4 text-xs text-left">
              <div>
                <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Kiasi (TSh) *</label>
                <input type="number" required value={payAmount} onChange={(e) => setPayAmount(e.target.value)} placeholder="50000" min="1" className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-accent focus:border-accent" />
              </div>
              <div>
                <label className="block font-semibold text-slate-500 uppercase tracking-wide mb-1">Notes</label>
                <input type="text" value={payNotes} onChange={(e) => setPayNotes(e.target.value)} placeholder="Maelezo ya malipo..." className="w-full p-2.5 border border-slate-200 rounded-xl focus:ring-accent focus:border-accent" />
              </div>
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 space-y-1.5">
                <div className="flex justify-between text-[11px]">
                  <span className="text-slate-400">Jumla ya Deni:</span>
                  <span className="font-bold text-slate-700">TSh {(selectedSupplier.amount || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-[11px]">
                  <span className="text-slate-400">Tayari Kulipwa:</span>
                  <span className="font-bold text-emerald-600">TSh {(selectedSupplier.paidAmount || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-[11px] border-t border-slate-200 pt-1.5">
                  ...
                  <span className="font-bold text-rose-600">TSh {Math.max(0, (selectedSupplier.amount || 0) - (selectedSupplier.paidAmount || 0) - Number(payAmount || 0)).toLocaleString()}</span>
                </div>
              </div>
              <div className="pt-2 flex justify-end gap-2">
                <button type="button" onClick={() => { setIsPayModalOpen(false); setSelectedSupplier(null); }} disabled={isLoading} className="px-4 py-2 bg-slate-50 hover:bg-slate-100 rounded-xl font-semibold text-slate-600 transition disabled:opacity-50">Ghairi</button>
                <button type="submit" disabled={isLoading || !payAmount || Number(payAmount) <= 0} className="px-5 py-2 bg-accent hover:bg-accent/90 text-white rounded-xl font-semibold shadow-sm transition disabled:opacity-50 flex items-center gap-2">
