...
export default function SupplierManagement({
  ...
  // Form states - Supplier (amount and dueDate are now OPTIONAL)
  ...
  const handleAddSupplier = async (e: React.FormEvent) => {
    ...
    try {
      await api.suppliers.create({
        ...
        phoneNumber,
        address: address || '',
        amount: 0, // Default 0, will be added later via products
        paidAmount: 0,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* MODAL 1: Add Supplier - Simplified (No amount required) */}
      {isAddModalOpen && (
      ...
      {/* MODAL 3: Pay Supplier - KEEP AS IS (only shows when there are debts) */}
      {/* MODAL 4: Add Supplier Product - KEEP AS IS */}
      {/* MODAL 5: Statement - KEEP AS IS */}
      {/* These modals remain the same as the original code */}

      {isPayModalOpen && selectedSupplier && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4">
          {/* ... same pay modal as before ... */}
        </div>
      )}

      {isAddProductModalOpen && activeSupplier && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 flex items-center justify-center p-4">
          {/* ... same add product modal as before ... */}
        </div>
      )}
      ...
      {isStatementOpen && activeSupplier && activeSupplierStats && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 flex items-center justify-center p-4">
          {/* ... same statement modal as before ... */}
        </div>
      )}
