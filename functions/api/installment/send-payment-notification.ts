// functions/api/installment/send-payment-notification.ts
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  try {
    ...
    const BUSINESS_NAME = env.BUSINESS_NAME || 'Sonko Sound';
    ...
    if (isCompleted) {
      // ✅ COMPLETION MESSAGES
      customerMessage = `Hongera ${customerName}! Umemaliza malipo ya ${productName} ya TSh ${totalAmount.toLocaleString()}. Bidhaa iko tayari kukabidhiwa. Asante kwa kuaminiana na ${BUSINESS_NAME}!`;
      ...
      // 📊 HALFWAY MILESTONE
      customerMessage = `Habari ${customerName}, umefika nusu ya malipo ya ${productName} (${progressPercentage}%). Umelipa TSh ${paidAmount.toLocaleString()}, baki TSh ${remaining.toLocaleString()}. Endelea hivyo hivyo!`;
      
      ownerMessage = `📊 ${customerName} amefika ${progressPercentage}% ya malipo ya ${productName}. Amelipa TSh ${paidAmount.toLocaleString()}, baki TSh ${remaining.toLocaleString()}.`;
      
    } else {
      // 💰 REGULAR PAYMENT
      customerMessage = `Habari ${customerName}, malipo ya TSh ${paymentAmount.toLocaleString()} ya ${productName} yamepokelewa. Kiwango kilicho baki ni TSh ${remaining.toLocaleString()}.`;
      
      ownerMessage = `💰 ${customerName} amelipa TSh ${paymentAmount.toLocaleString()} ya ${productName} kupitia ${paymentMethod || 'Cash'}. Kiwango kilicho baki ni TSh ${remaining.toLocaleString()}.`;
    }

    console.log('📦 INSTALLMENT SMS:');
    console.log('  Type:', isCompleted ? 'COMPLETED' : progressPercentage >= 45 && progressPercentage <= 55 ? 'HALFWAY' : 'PAYMENT');
    console.log('  To Customer:', customerMessage);
    console.log('  To Owner:', ownerMessage);
    ...
    return json({
      ...
      data: {
        ...
        customerMessage,
        ownerMessage,
        type: isCompleted ? 'completed' : progressPercentage >= 45 && progressPercentage <= 55 ? 'halfway' : 'payment',
      },
